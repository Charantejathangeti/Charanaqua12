          ...
          <div className="flex items-center space-x-5">
            ...
            {auth.isAuthenticated ? (
              <div className="relative group">
                ...
                {/* Dropdown */}
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2 hidden group-hover:block text-gray-800 border border-gray-100 transform origin-top-right transition">
                    <div className="px-4 py-2 border-b border-gray-100">
                      <p className="text-xs text-gray-500">Signed in as</p>
                      <p className="font-bold truncate">{auth.user?.name}</p>
                    </div>
                    <Link to="/orders" className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 text-gray-700">My Orders</Link>
                    <button onClick={logout} className="block w-full text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600">Logout</button>
                </div>
              </div>
            ) : (
          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 text-sm">
              <li className="flex items-start space-x-3">
                ...
                <span>{STORE_DETAILS.address}<br />Tirupati - {STORE_DETAILS.pincode}</span>
              </li>
              <li className="flex items-center space-x-3">
                <Phone className="h-5 w-5 text-brand-cyan shrink-0" />
                <span className="text-lg font-semibold text-white">+91 {STORE_DETAILS.phone}</span>
              </li>
